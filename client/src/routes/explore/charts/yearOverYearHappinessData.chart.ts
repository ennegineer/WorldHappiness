      ...
      datasets: [
        {
          ...
          data: input.map((row: any) => (row.confidence_in_national_government * 100).toFixed(2))
        },
        {
          label: 'Freedom to Make Life Choices',
          data: input.map((row: any) => (row.freedom_to_make_life_choices * 100).toFixed(2))
        },
        {
          label: 'GDP Per Capita (nat. log)',
          data: input.map((row: any) => row.log_gdp_per_capita)
        },
        {
          label: 'Positive Affect',
          data: input.map((row: any) => (row.positive_affect * 100).toFixed(2))
        },
        {
          label: 'Negative Affect',
          data: input.map((row: any) => (row.negative_affect * 100).toFixed(2))
        },
        {
          label: 'Generosity',
          data: input.map((row: any) => (row.generosity * 100).toFixed(2))
        },
        {
          label: 'Perceptions Of Corruption',
          data: input.map((row: any) => (row.perceptions_of_corruption * 100).toFixed(2))
        },
        {
          label: 'Social Support',
          data: input.map((row: any) => (row.social_support * 100).toFixed(2))
        }
      ]
    },
    options: {
